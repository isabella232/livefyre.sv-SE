...
| checksum | *rekommenderad* sträng (rekommenderas) | Identifierar det aktuella tillståndet för CollectionMeta. Om du ändrar det här värdet kommer Livefyre att uppdatera data på servern med de nya värdena i CollectionMeta. |
| datetimeFormat | *valfri* strängobjektsfunktion | Anger datum/tid-formatet för direktuppspelat innehåll. Mer information finns i Anpassa datum- och tidsstämplar. |
...
| disableThirdPartyAnalytics | *optional* bBoolean | Inaktiverar analyssystem från tredje part (Quantserve och Google Analytics) som Livefyre kan använda för interna mätningar. Standardvärdet är false. |
