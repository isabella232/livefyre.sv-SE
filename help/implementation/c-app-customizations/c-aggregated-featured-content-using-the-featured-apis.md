...
Livefyre innehåller fyra API:er med funktioner, som gör att du kan hämta de 50 senast använda innehållsobjekten för indatanätverket, platsen eller samlingen, eller de två innehållsdelarna som finns högst upp i en indatamängd.

## Aktuellt (nätverk) {#section_cgm_1nw_xz}

Hämtar de 50 senaste delarna av aktuellt innehåll (och dess författare) för indatanätverket. (Använd sidindelning för att returnera allt innehåll, i stället för bara de senaste 50.)
...
## Aktuell (samling) {#section_kgc_xmw_xz}

Returnerar de 50 senaste delarna av aktuellt innehåll (och dess författare) för indatamängden. (Använd sidindelning för att returnera allt innehåll, i stället för bara de senaste 50.)
